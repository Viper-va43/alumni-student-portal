...
if (($_SERVER['REQUEST_METHOD'] ?? 'GET') === 'POST' && ($_POST['action'] ?? '') === 'claim_checkin') {
    if (!verify_csrf_token($_POST['csrf_token'] ?? '')) {
        $messages[] = [
            'type' => 'error',
            'text' => 'Your session expired. Please refresh the page and try again.',
        ];
    } elseif (!$loggedIn) {
...
<meta name="description" content="Use Where2Go QR check-in to claim eligible location rewards after visiting partner places.">
