...
$postCsrfOk = !$isPostRequest || verify_csrf_token($_POST['csrf_token'] ?? '');

if ($isPostRequest && !$postCsrfOk) {
...
function build_business_detail_media_items($business) {
    ...
    if (!empty($business['theme_cover_url'])) {
        $urls[] = (string) $business['theme_cover_url'];
    }
...
if ($businessId > 0) {
    ...
    if ($selectedLocation) {
        ...
        $selectedGuests = max($selectedGuests, get_location_min_party_size($selectedLocation));
    }

    if ($postCsrfOk && (($_SERVER['REQUEST_METHOD'] ?? 'GET') === 'POST') && ($_POST['action'] ?? '') === 'delete_review') {
        if (!$loggedIn) {
    ...
    if ($postCsrfOk && (($_SERVER['REQUEST_METHOD'] ?? 'GET') === 'POST') && ($_POST['action'] ?? '') === 'submit_review') {
        if (!$loggedIn) {
    ...
    if ($postCsrfOk && (($_SERVER['REQUEST_METHOD'] ?? 'GET') === 'POST') && ($_POST['action'] ?? '') === 'book_location') {
        if (!$loggedIn) {
...
// Fall back to the built-in curated catalog place details when no business id is supplied.
    ...
    if ($postCsrfOk && (($_SERVER['REQUEST_METHOD'] ?? 'GET') === 'POST') && ($_POST['action'] ?? '') === 'delete_catalog_review') {
        if (!$loggedIn) {
            $messages[] = ['type' => 'error', 'text' => 'Login with a customer account before deleting a review.'];
        } else {
    ...
    if ($postCsrfOk && (($_SERVER['REQUEST_METHOD'] ?? 'GET') === 'POST') && ($_POST['action'] ?? '') === 'submit_catalog_review') {
        if (!$loggedIn) {
            $messages[] = ['type' => 'error', 'text' => 'Login with a customer account before leaving a review.'];
        } else {
...
$businessTheme = $source === 'business' ? get_business_theme_payload($business) : null;
$businessThemePreset = $businessTheme ? normalize_business_theme_preset($businessTheme['preset'] ?? 'where2go') : '';
$businessThemeAccent = $businessTheme ? normalize_business_theme_accent_color($businessTheme['accentColor'] ?? '') : '';
?>
<!DOCTYPE html>
<html lang="en">
<head>
...
<?php if ($businessTheme): ?>
<style>
body.business-themed {
    --business-accent: <?php echo htmlspecialchars($businessThemeAccent ?: '#F26C1C', ENT_QUOTES, 'UTF-8'); ?>;
}

body.business-themed .hero-panel {
    border-color: color-mix(in srgb, var(--business-accent) 42%, var(--border));
    background:
        linear-gradient(135deg, color-mix(in srgb, var(--business-accent) 24%, transparent), transparent 48%),
        var(--panel);
}

body.business-themed .hero-chip,
body.business-themed .business-tagline,
body.business-themed .reservation-heading-icon {
    color: var(--business-accent);
}

body.business-themed .primary-btn,
body.business-themed .hero-button.primary,
body.business-themed .availability-slot input:checked + span {
    background: var(--business-accent);
    border-color: var(--business-accent);
}

body.business-themed .gallery-main {
    border-color: color-mix(in srgb, var(--business-accent) 34%, var(--border));
}

body.business-theme-luxury .hero-panel {
    background: linear-gradient(135deg, rgba(218, 165, 32, 0.22), rgba(20, 12, 8, 0.88));
}

body.business-theme-nightlife .hero-panel {
    background: linear-gradient(135deg, rgba(242, 108, 28, 0.22), rgba(17, 9, 30, 0.92));
}

body.business-theme-family .hero-panel {
    background: linear-gradient(135deg, rgba(255, 173, 90, 0.22), rgba(39, 27, 16, 0.78));
}

body.business-theme-cafe .hero-panel {
    background: linear-gradient(135deg, rgba(166, 111, 63, 0.24), rgba(38, 22, 12, 0.84));
}

body.business-theme-minimal .hero-panel {
...
.business-style-pill {
    ...
    padding: 8px 12px;
    border-radius: 999px;
    background: color-mix(in srgb, var(--business-accent) 14%, transparent);
    color: var(--business-accent);
    font-weight: 800;
}
...
<body class="dark-mode<?php echo $businessTheme ? ' business-themed business-theme-' . htmlspecialchars($businessThemePreset, ENT_QUOTES, 'UTF-8') : ''; ?>">
...
<main class="main-inner">
    ...
    <section class="hero-panel">
        ...
        <?php if ($businessTheme && trim((string) ($businessTheme['tagline'] ?? '')) !== ''): ?>
        <p class="business-tagline"><?php echo htmlspecialchars((string) $businessTheme['tagline'], ENT_QUOTES, 'UTF-8'); ?></p>
        <?php endif; ?>
        <p class="hero-copy" id="detail-summary"><?php echo htmlspecialchars($pageSummary, ENT_QUOTES, 'UTF-8'); ?></p>
        <?php if ($businessTheme && $businessThemePreset !== 'where2go'): ?>
        <span class="business-style-pill"><i data-lucide="palette" style="width:14px;height:14px;"></i><?php echo htmlspecialchars((string) ($businessTheme['label'] ?? 'Business style'), ENT_QUOTES, 'UTF-8'); ?></span>
        <?php endif; ?>
            ...
            <aside class="detail-side">
                ...
                <div class="detail-panel reservation-panel">
                    ...
                    <form action="place.php?business_id=<?php echo $businessId; ?>" method="GET" class="reservation-controls">
                        ...
                        <label class="reservation-field">
                            <span>Guests</span>
                        <input type="number" name="guests" min="<?php echo get_location_min_party_size($selectedLocation); ?>" max="<?php echo get_location_guest_limit($selectedLocation); ?>" value="<?php echo $selectedGuests; ?>">
                        </label>
