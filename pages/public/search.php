...
function build_search_filter_text($place) {
    ...
    return normalize_place_catalog_token(implode(' ', array_filter([
        ...
        $place['description'] ?? '',
        $place['search_tags'] ?? '',
        is_array($place['tags'] ?? null) ? implode(' ', $place['tags']) : '',
        $place['search_blob'] ?? '',
    ])));
}
...
function render_search_result_card($place, $loggedIn, $visitedLookup) {
    ...
    $searchTags = is_array($place['tags'] ?? null) ? array_slice($place['tags'], 0, 4) : [];
    $payload = [];
    ...
    <article class="result-card" data-result-href="<?php echo htmlspecialchars($detailUrl, ENT_QUOTES, 'UTF-8'); ?>">
        ...
        <div class="result-tags">
            ...
            <span class="tag"><i data-lucide="tag" style="width:14px;height:14px;"></i><?php echo htmlspecialchars((string) $tag, ENT_QUOTES, 'UTF-8'); ?></span>
            <?php endforeach; ?>
        </div>
