...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!$errors) {
        $rateLimit = get_login_rate_limit_status('customer', $email);
    ...
    if (!$errors) {
        try {
            ...
            if (!$customer || !verify_password($password, $customer['Password'])) {
                record_login_attempt('customer', $email, false);
                $errors[] = 'The email or password is incorrect.';
            } else {
                clear_login_attempts('customer', $email);
                record_login_attempt('customer', $email, true);
...
<meta name="description" content="Sign in to Where2Go to save places, manage your profile, write reviews, and use rewards.">
