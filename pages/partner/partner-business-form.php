...
function build_partner_form_view($postData, $businessId = 0) {
    ...
    return [
        'business' => [
            ...
            'search_tags' => normalize_business_search_tags($postData['search_tags'] ?? ''),
            'logo_url' => trim((string) ($postData['logo_url'] ?? '')),
            'website' => trim((string) ($postData['website'] ?? '')),
            'theme_preset' => normalize_business_theme_preset($postData['theme_preset'] ?? 'where2go'),
            'theme_accent_color' => normalize_business_theme_accent_color($postData['theme_accent_color'] ?? ''),
            'theme_cover_url' => trim((string) ($postData['theme_cover_url'] ?? '')),
            'brand_tagline' => trim((string) ($postData['brand_tagline'] ?? '')),
...
function render_partner_location_card($index, $location) {
    ...
    <div class="repeat-card location-card" data-location-card data-dynamic-index="<?php echo (int) $index; ?>">
        ...
        <div class="panel-card" style="padding:18px; margin-bottom:18px;">
            <div class="dashboard-item-head">
                <div>
                    <h3 style="margin:0 0 6px;">Reservation rules</h3>
                    <p class="mini-note" style="margin:0;">Control party size, booking spacing, same-day cutoff, and days that should not accept reservations.</p>
                </div>
            </div>
            <div class="grid-three">
                <div class="field">
                    <label for="location_min_party_<?php echo (int) $index; ?>">Minimum party size</label>
                    <input id="location_min_party_<?php echo (int) $index; ?>" type="number" min="1" name="locations[<?php echo (int) $index; ?>][min_party_size]" value="<?php echo max(1, (int) ($location['min_party_size'] ?? 1)); ?>">
                </div>
                <div class="field">
                    <label for="location_max_party_<?php echo (int) $index; ?>">Maximum party size</label>
                    <input id="location_max_party_<?php echo (int) $index; ?>" type="number" min="1" name="locations[<?php echo (int) $index; ?>][max_party_size]" value="<?php echo max(1, (int) ($location['max_party_size'] ?? 40)); ?>">
                </div>
                <div class="field">
                    <label for="location_duration_<?php echo (int) $index; ?>">Reservation duration</label>
                    <input id="location_duration_<?php echo (int) $index; ?>" type="number" min="15" step="15" name="locations[<?php echo (int) $index; ?>][reservation_duration_minutes]" value="<?php echo max(15, (int) ($location['reservation_duration_minutes'] ?? 60)); ?>">
                    <p class="mini-note" style="margin:0;">Minutes per booking.</p>
                </div>
            </div>
            <div class="grid-three" style="margin-top:14px;">
                <div class="field">
                    <label for="location_buffer_<?php echo (int) $index; ?>">Buffer time</label>
                    <input id="location_buffer_<?php echo (int) $index; ?>" type="number" min="0" step="5" name="locations[<?php echo (int) $index; ?>][reservation_buffer_minutes]" value="<?php echo max(0, (int) ($location['reservation_buffer_minutes'] ?? 0)); ?>">
                    <p class="mini-note" style="margin:0;">Minutes between booking starts.</p>
                </div>
                <div class="field">
                    <label for="location_cutoff_<?php echo (int) $index; ?>">Same-day cutoff</label>
                    <input id="location_cutoff_<?php echo (int) $index; ?>" type="time" name="locations[<?php echo (int) $index; ?>][same_day_cutoff_time]" value="<?php echo htmlspecialchars(substr((string) ($location['same_day_cutoff_time'] ?? ''), 0, 5), ENT_QUOTES, 'UTF-8'); ?>">
                    <p class="mini-note" style="margin:0;">Leave empty to allow same-day requests until slots close.</p>
                </div>
                <div class="field">
                    <span>Approval mode</span>
                    <label class="checkbox-row">
                        <input type="checkbox" name="locations[<?php echo (int) $index; ?>][auto_approve_reservations]" value="1"<?php echo !empty($location['auto_approve_reservations']) ? ' checked' : ''; ?>>
                        <span>Auto-approve reservations</span>
                    </label>
                </div>
            </div>
            <div class="field" style="margin-top:14px;">
                <label for="location_blocked_dates_<?php echo (int) $index; ?>">Blocked dates</label>
                <textarea id="location_blocked_dates_<?php echo (int) $index; ?>" name="locations[<?php echo (int) $index; ?>][blocked_dates]" placeholder="2026-07-01&#10;2026-07-02"><?php echo htmlspecialchars((string) ($location['blocked_dates'] ?? ''), ENT_QUOTES, 'UTF-8'); ?></textarea>
                <p class="mini-note" style="margin:0;">Use one date per line or separate dates with commas. These dates will show as unavailable.</p>
            </div>
        </div>
...
$typeOptions = [
    ...
    'heritage' => ['label' => 'Heritage & Culture', 'icon' => 'landmark'],
...
<meta name="description" content="Create or edit a Where2Go partner business listing with locations, media, offers, menus, and rules.">
    ...
    <form action="partner-business-form.php<?php echo $businessId > 0 ? '?business_id=' . $businessId : ''; ?>" method="POST" class="form-grid" style="margin-top:24px;">
        ...
        <section class="panel-card">
            ...
            <div class="field">
                ...
                <input id="search_tags" type="text" name="search_tags" value="<?php echo htmlspecialchars((string) ($business['search_tags'] ?? ''), ENT_QUOTES, 'UTF-8'); ?>" placeholder="date night, rooftop, family friendly, budget, live music">
                <p class="mini-note" style="margin:0;">Separate tags with commas. Customers can find your place when they search these moods, features, or occasions.</p>
            </div>
        ...
        <section class="panel-card">
            <h2>Business page style</h2>
            <p class="section-copy">Customize the feeling of your public page while Where2Go keeps the booking, reviews, and layout consistent.</p>
            <div class="grid-two">
                <div class="field">
                    <label for="theme_preset">Style preset</label>
                    <select id="theme_preset" name="theme_preset">
                        <?php foreach (get_business_theme_presets() as $presetValue => $presetLabel): ?>
                        <option value="<?php echo htmlspecialchars($presetValue, ENT_QUOTES, 'UTF-8'); ?>"<?php echo normalize_business_theme_preset($business['theme_preset'] ?? 'where2go') === $presetValue ? ' selected' : ''; ?>><?php echo htmlspecialchars($presetLabel, ENT_QUOTES, 'UTF-8'); ?></option>
                        <?php endforeach; ?>
                    </select>
                </div>
                <div class="field">
                    <label for="theme_accent_color">Accent color</label>
                    <input id="theme_accent_color" type="color" name="theme_accent_color" value="<?php echo htmlspecialchars(normalize_business_theme_accent_color($business['theme_accent_color'] ?? '') ?: '#F26C1C', ENT_QUOTES, 'UTF-8'); ?>">
                </div>
            </div>

            <div class="field">
                <label for="theme_cover_url">Cover image URL</label>
                <input id="theme_cover_url" type="url" name="theme_cover_url" value="<?php echo htmlspecialchars((string) ($business['theme_cover_url'] ?? ''), ENT_QUOTES, 'UTF-8'); ?>" placeholder="https://...">
                <p class="mini-note" style="margin:0;">This becomes the main branded image when a customer opens your business.</p>
            </div>

            <div class="field">
                <label for="brand_tagline">Short brand tagline</label>
                <input id="brand_tagline" type="text" maxlength="140" name="brand_tagline" value="<?php echo htmlspecialchars((string) ($business['brand_tagline'] ?? ''), ENT_QUOTES, 'UTF-8'); ?>" placeholder="Fine dining on the Nile, family-friendly fun, rooftop nights...">
            </div>
        </section>
        ...
        <section class="panel-card">
            ...
            <div class="photo-order-list" data-photo-list>
                <?php foreach ($photoRows as $index => $photoUrl): ?>
                <div class="photo-order-row" data-photo-row draggable="true">
                    <button class="photo-drag-handle" type="button" data-drag-handle aria-label="Drag photo <?php echo $index + 1; ?>">
                    ...
                    <div class="photo-order-actions">
                        <button class="photo-order-btn" type="button" data-move-photo="up" aria-label="Move photo <?php echo $index + 1; ?> up"><i data-lucide="chevron-up"></i></button>
                        <button class="photo-order-btn" type="button" data-move-photo="down" aria-label="Move photo <?php echo $index + 1; ?> down"><i data-lucide="chevron-down"></i></button>
                    </div>
                </div>
                <?php endforeach; ?>
            </div>
