...
if (($_SERVER['REQUEST_METHOD'] ?? 'GET') === 'POST') {
    ...
    if (!$errors) {
        $rateLimit = get_login_rate_limit_status('partner', $email);

        if (!empty($rateLimit['limited'])) {
            $waitMinutes = max(1, (int) ceil(((int) $rateLimit['retry_after_seconds']) / 60));
            $errors[] = 'Too many failed login attempts. Try again in about ' . $waitMinutes . ' minutes.';
        }
    }
    ...
    if (!$errors) {
        ...
        if (!$partner || !verify_password($password, (string) ($partner['password'] ?? ''))) {
            record_login_attempt('partner', $email, false);
            $errors[] = 'The partner email or password is incorrect.';
        } else {
            clear_login_attempts('partner', $email);
            record_login_attempt('partner', $email, true);
...
<meta name="description" content="Sign in to the Where2Go partner portal to manage business listings, locations, offers, and reservations.">
